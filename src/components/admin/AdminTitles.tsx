...
export const AdminTitles = () => {
    ...
    const [deleteItem, setDeleteItem] = useState<Data<DocumentData> | undefined>()
    ...
    useEffect(() => {
        snapshot?.map((item) => {
            if (featuredCollection && featuredCollection.length >= 1 && item.id == featuredCollection[0].id) {
    ...
    const saveItem = () => {
        if (editItem.titleSwedish && editItem.imgUrl && editItem.category && editItem.descriptionSV && editItem.category) {
    ...
    const thumbnails = snapshot?.map((title, i) => {
        return (
            <div key={i} className="cardContainer">
                ...
                <div>
                    <Button className="buttons" variant="outlined" onClick={() => setEditItem(title as unknown as ITitle)}>Edit</Button>
                    <Button className="buttons" variant="outlined" color="warning" onClick={() => setDeleteItem(title)}>Delete</Button>
                </div>
            </div >
        )
    })
    ...
    return (
        <div className="adminCardContainer">
            <Fab className="addButton" href="/admin/add" color="primary">add</Fab>

            {thumbnails}

            {/* EDIT FORM */}
            {editItem ? <Dialog open={editItem == defaultTitle ? false : true} onClose={() => setEditItem(defaultTitle)}>
                ...
                <DialogActions>
                    {
                        featuredTitle?.titleSwedish == editItem.titleSwedish ?
                            <Button onClick={() => removeFeature(editItem as unknown as Data<DocumentData>)} color="warning">{"Remove feature"}</Button>
                            : <Button onClick={() => setFeaturedDB(editItem as unknown as Data<DocumentData>)}>{"Set feature"}</Button>
                    }
                    <Button onClick={() => setEditItem(defaultTitle)}>Cancel</Button>
                    <Button variant="outlined" onClick={saveItem}>Save</Button>
                </DialogActions>
            </Dialog>
                : null}

            {/* DELETE FORM */}
            {deleteItem ? <Dialog open={deleteItem ? true : false} onClose={() => { setDeleteItem(undefined); setDeleteForm("") }}>
                <DialogTitle>Delete {deleteItem.titleSwedish}</DialogTitle>
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete <strong>{deleteItem.titleSwedish}</strong>?
                        <br />
                ...
                <DialogActions>
                    ...
                    <Button color="warning" variant="contained" onClick={() => deleteTitle(deleteItem)}>Delete</Button>
                </DialogActions>
            </Dialog>
                : null}
        </div>
